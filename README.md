...
- **TTS: Local Piper** uses the same endpoint with `engine=piper`. The container includes a default `en_GB-southern_english_female-low` Piper voice. To use a custom Piper voice, set `PIPER_MODEL` to a host `.onnx` file path before running `deploy.sh`; the script mounts that file's directory read-only at `/custom_voice` and points the container at the mounted model. For example: `PIPER_MODEL=/home/me/voices/my_voice.onnx ./deploy.sh 3014`. Keep the matching `.onnx.json` config file beside the model file. As a shortcut, placing `custom_voice/my_voice.onnx` and `custom_voice/my_voice.onnx.json` in this repo automatically mounts that directory and sets `PIPER_MODEL=/custom_voice/my_voice.onnx`; do not hard-code `-e PIPER_MODEL=/custom_voice/my_voice.onnx` unless you also mount the directory.
